...
export function BoardSpace({ space, status, players, position, onClick }: BoardSpaceProps) {
  const owner = status?.ownerId ? players.find((player) => player.id === status.ownerId) : undefined;
  ...
  return (
    <motion.button
      type="button"
      className={`board-space side-${side} ${cornerIds.has(space.id) ? "corner-space" : ""} ${actionClass} ${status?.mortgaged ? "is-mortgaged" : ""}`}
    ...
    >
      ...
      {status?.ownerId && <span className="ownership-strip" style={{ backgroundColor: owner?.color ?? "#7c4dff" }} />}
